...
public class BlackjackTest extends TestCase {
    ...
    public void testP() {
        Assert.assertEquals(5*4*3, Ma.p(5,3));
    }

    public void testC() {
        Assert.assertEquals(10*9*8*7/(4*3*2*1), Ma.c(10,4));
    }

    public void testIsStraight() {
        List<Pocker> pockers = Arrays.asList(Pocker.getPocker(4), Pocker.getPocker(5), Pocker.getPocker(6));
        Assert.assertTrue(Ma.isStraight(pockers));
        log.info("{}",pockers);
        ...
        log.info("{}",pockers);
        Assert.assertTrue(Ma.isStraight(pockers));
        Assert.assertTrue(Ma.isFlush(pockers));
